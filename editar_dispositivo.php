...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($tipo_err) && empty($marca_err) && empty($modelo_err) && empty($fecha_entrega_err) && empty($estado_err)){
        $sql = "UPDATE dispositivos SET tipo=?, marca=?, modelo=?, fecha_entrega=?, estado=? WHERE id=? AND id_usuario=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "sssssii", $param_tipo, $param_marca, $param_modelo, $param_fecha_entrega, $param_estado, $param_id, $param_id_usuario);
    ...
    $sql = "SELECT * FROM dispositivos WHERE id = ? AND id_usuario = ?";
    if($stmt = mysqli_prepare($link, $sql)){
        mysqli_stmt_bind_param($stmt, "ii", $param_id, $param_id_usuario);
...
<head>
    ...
    <title>Editar Dispositivo - Control Stock</title>
